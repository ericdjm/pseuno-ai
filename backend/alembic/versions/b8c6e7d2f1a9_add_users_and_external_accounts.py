...
def upgrade() -> None:
    ...
    # Postgres-only: create trigger function and triggers for updated_at
    # SQLite doesn't support triggers in the same way, so we skip this
    bind = op.get_bind()
...
def downgrade() -> None:
    ...
    # Postgres-only: drop triggers and function
    bind = op.get_bind()
    if bind.dialect.name == "postgresql":
        op.execute(
